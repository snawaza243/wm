...
BEGIN
    ...
    BEGIN -- Check for duplicate PAN globally
        SELECT count(CLIENT_PAN) INTO VPAN FROM WEALTHMAKER.CLIENT_TEST
        WHERE UPPER(TRIM(SUBSTR(CLIENT_PAN, 1, 10))) = UPPER(TRIM(SUBSTR(P_CLIENT_PAN, 1, 10))) AND CLIENT_CODE != P_SEARCH_CLIENT_CODE;
        EXCEPTION WHEN NO_DATA_FOUND THEN VPAN := 0;
    END;
    
    
    BEGIN -- get family sourcecode 
        SELECT SOURCE_CODE INTO V_SOURCE_CODE FROM WEALTHMAKER.CLIENT_TEST 
        WHERE CLIENT_CODE = P_SEARCH_CLIENT_CODE;
        EXCEPTION WHEN NO_DATA_FOUND THEN V_SOURCE_CODE := NULL;
    END;
    
      -- validate mobile no of family sourcecode 
    BEGIN
        SELECT COUNT(MOBILE_NO) INTO VMOBILE FROM WEALTHMAKER.CLIENT_TEST
        WHERE MOBILE_NO = P_MOBILE_NO_VALUE AND SOURCE_CODE != V_SOURCE_CODE;  -- Ensure it's not the same family
        EXCEPTION WHEN NO_DATA_FOUND THEN VMOBILE := 0;
    END;
    
    -- Check if the same email exists for a different family (SOURCE_CODE)
    BEGIN
        SELECT COUNT(EMAIL) INTO VEMAIL FROM WEALTHMAKER.CLIENT_TEST
    ...
    BEGIN
        SELECT COUNT(G_PAN) INTO VG_PAN FROM WEALTHMAKER.CLIENT_TEST
        WHERE UPPER(G_PAN) = UPPER(P_GUARDIAN_PERSON_PAN) AND SOURCE_CODE != V_SOURCE_CODE;  -- Ensure it's not the same family

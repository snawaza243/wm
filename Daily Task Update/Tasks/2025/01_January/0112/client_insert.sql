...
SELECT 
check_dt_newinv(P_DT_NUMBER) into DT_FLAG 
FROM dual;


-- Validate DT number is not null and valid
IF TRIM(P_DT_NUMBER) IS NOT NULL THEN
    -- Check if the DT number is valid using the function
    IF (DT_FLAG) <= 0 THEN
        OPEN P_RESULT FOR
    ...
    END IF;
ELSE
    -- If the DT number is not null, show a message
    OPEN P_RESULT FOR
...
IF TRIM(P_CLIENT_PAN) IS NOT NULL THEN
        SELECT COUNT(*) INTO VCNT FROM INVESTOR_MASTER WHERE PAN=P_CLIENT_PAN;
        IF VCNT >=1 THEN
            OPEN P_RESULT FOR
                SELECT 'Duplicate INVESTOR Pan No' AS message FROM DUAL;
                return;
        END IF;
ELSE
    -- If the DT number is not null, show a message
...
IF LENGTH(TRIM(P_EXIST_CLIENT_CODE)) >= 11 THEN
 ...
 SET
    ...
    WHERE INV_CODE = P_EXIST_CLIENT_CODE;
        ...
        OPEN P_RESULT FOR
            SELECT 'Account created succeccfully --> ' || v_generated_client_code  AS message FROM DUAL;
        RETURN;
...
COMMIT;
        ...
        OPEN P_RESULT FOR
            SELECT 'Data Inserted succeccfully --> ' || V_MYSOURCEID || ' | '  || 

CREATE OR REPLACE PROCEDURE WEALTHMAKER.PRC_GETRTADETAILS ( 
...
*/
     ...
     VSQL := VSQL || ' AND (T.ASA <> ''C'' OR T.ASA IS NULL) AND T.TRAN_TYPE IN (''PURCHASE'', ''REINVESTMENT'',''SWITCH IN'') ORDER BY  TR_DATE,RM_NAME) GROUP BY FOLIO_NO,SCH_CODE,TR_DATE,TRAN_TYPE ';
        ...
        OPEN P_CURSOR FOR select VSQL as ERROR_COL from dual;
        
EXCEPTION
     WHEN OTHERS
     THEN
     P_ERRORMESSAGE := SQLERRM (SQLCODE);
     OPEN P_CURSOR FOR SELECT '--' || P_ERRORMESSAGE || '--' AS ERROR_COL FROM DUAL;  

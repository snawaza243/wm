...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    .grid-view tr.highlighted {
        background-color: #e3f2fd !important; /* Light blue background */
        font-weight: bold;
    }
    
    .grid-view tr.highlighted td {
        border-top: 2px solid #2196F3 !important;
        border-bottom: 2px solid #2196F3 !important;
    }
    
    /* Selected link button style */
    .selected-link {
        color: #fff !important;
    ...
    <script type="text/javascript">
        // Maintain selection after async postback
        var prm = Sys.WebForms.PageRequestManager.getInstance();

        prm.add_endRequest(function () {
            // Re-apply highlighting after async postback
            var selectedTranCode = '<%= Session["TRCode"] != null ? Session["TRCode"].ToString() : "" %>';
        if (selectedTranCode !== '') {
            $('[id*="GridView1"] tr').each(function () {
                var row = $(this);
                var tranCode = row.find('td:eq(1)').text().trim(); // Adjust index based on TRAN_CODE column position
                if (tranCode === selectedTranCode) {
                    row.addClass('highlighted');
                    var lnk = row.find('[id*="lnkSelect"]');
                    lnk.text('Selected');
                    lnk.removeClass('btn-outline-primary').addClass('btn-primary selected-link');
                }
            });
        }
    });
                    ...
                    <ContentTemplate>
                        <div>
                            ...
                            <div class="table-responsive gridViewContainerRTA">
                                <asp:GridView ID="GridView2" runat="server" CssClass="grid-view table table-hover"></asp:GridView>
                            </div>
                        </div>

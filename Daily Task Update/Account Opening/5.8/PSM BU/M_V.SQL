...
IF TRIM(P_EXISTING_MEMBER_INV) IS NULL THEN
    BEGIN
        IF p_mobile IS NOT NULL AND UPPER(p_head_nationality) = 'NO' THEN -- VALIDATE MOBILE AND RETURN INVALID, DUPLICATE

            ISVALIDMOBILE := VALIDATE_MOBILE(P_MOBILE);

            IF ISVALIDMOBILE = 0 THEN
                OPEN P_RESULT FOR SELECT ' in Invalid Mobile Number (e.g. 9999999999)' || p_head_nationality AS MESSAGE FROM DUAL;
                RETURN;    
            ELSIF ISVALIDMOBILE = 1  THEN
                SELECT NVL(COUNT(MOBILE) ,0) INTO VMOBILE 
                FROM WEALTHMAKER.INVESTOR_MASTER WHERE MOBILE = P_MOBILE AND SOURCE_ID != TRIM(SUBSTR(P_EXIST_CLIENT_CODE,1,8));   
                IF VMOBILE > 0 THEN
        ...
        IF P_EMAIL IS NOT NULL THEN -- VALIDATE EMAIL AND RETURN INVALID, DUPLICATE
            IF UPPER(P_EMAIL) NOT IN ('NA', 'NOT AVAILABLE', 'N/A', 'NONE') THEN
                ...
                ELSIF ISVALIDEMAIL = 1  THEN 
                    ...
                    FROM WEALTHMAKER.INVESTOR_MASTER WHERE UPPER(EMAIL) = UPPER(P_EMAIL)
        ...
        ELSIF ISVALID_AADHAAR = 'Valid' THEN          
                ...
                FROM WEALTHMAKER.INVESTOR_MASTER
                WHERE AADHAR_CARD_NO = TRIM(SUBSTR(P_AADHAR_VALUE,1,12)) AND P_AADHAR_VALUE IS NOT NULL AND P_AADHAR_VALUE != 0;

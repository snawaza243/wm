    ...
    IF TRIM(P_GUEST_CODE) IS NOT NULL THEN
        ...
        ELSIF ISVALIDGUEST > 0  THEN
            ...
            IF V_GUESTCODE > 0 THEN -- Duplicate Guest Code found in CLIENT_MASTER
                OPEN P_RESULT FOR SELECT 'DUPLICATE GUEST CODE WITH ' || (SELECT client_code FROM CLIENT_MASTER WHERE GUEST_CD = trim(P_GUEST_CODE) AND ROWNUM = 1 ) AS MESSAGE FROM DUAL;
                RETURN;
            END IF;
        END IF;
    ELSE
    ...
    IF TRIM(P_EMAIL_VALUE) IS NOT NULL THEN
        IF UPPER(P_EMAIL_VALUE) NOT IN ('NA', 'NOT AVAILABLE', 'N/A', 'NONE') THEN
            ...
            ELSIF ISVALIDEMAIL > 0 THEN
                ...
                IF VEMAIL > 0 THEN
                    OPEN P_RESULT FOR 
                    SELECT 'Duplicate Email (Cannot be shared across different families) HEAD -->' || P_EXIST_CLIENT_CODE AS MESSAGE FROM DUAL; 
                    RETURN;
                END IF;
            END IF;
        END IF;

create or replace PROCEDURE PSM_AO_C_VALIDATE(
...
IF CURRENT_ROLE != 0 THEN
    OPEN P_RESULT FOR
        SELECT 'ACCESS: ONLY PUNCHING TEAM CAN PUNCH!' AS MESSAGE FROM DUAL;
    RETURN;
END IF;
...
BEGIN
    ...
    IF P_GUEST_CODE IS NOT NULL THEN 
        SELECT NVL(COUNT(GUEST_CD),0) INTO ISVALIDGUEST FROM BAJAJ_VENUE_BOOKING WHERE GUEST_CD = TRIM(P_GUEST_CODE);

        IF ISVALIDGUEST = 0 THEN
            OPEN P_RESULT FOR SELECT 'Guest Code does not exist' AS MESSAGE FROM DUAL;
            RETURN;

        ELSIF ISVALIDGUEST > 0  THEN
            SELECT NVL(COUNT(GUEST_CD),0) INTO V_GUESTCODE FROM CLIENT_MASTER 
            WHERE GUEST_CD = TRIM(P_GUEST_CODE) AND CLIENT_CODE  != SUBSTR(TRIM(P_EXIST_CLIENT_CODE),1,8);

            IF V_GUESTCODE > 0 THEN
            -- Duplicate Guest Code found in CLIENT_MASTER
                OPEN P_RESULT FOR SELECT 'Duplicate Guest Code ' AS MESSAGE FROM DUAL;
                RETURN;
            END IF;
        END IF;
    ELSE
        IF TRIM(P_RM_BUSINESS_CODE) IS  NOT NULL THEN
            ...
            IF V_GUEST_FPC_BRANCH > 0 THEN 
                OPEN P_RESULT FOR SELECT 'A valid guest code requried for FPC' AS MESSAGE FROM DUAL; 
                RETURN; 
            END IF;
        ELSE
            OPEN P_RESULT FOR 
    ...
    IF P_EMAIL_VALUE IS NOT NULL THEN
        IF UPPER(P_EMAIL_VALUE) NOT IN ('NA', 'NOT AVAILABLE', 'N/A', 'NONE') THEN
            ...
            IF ISVALIDEMAIL = 0 THEN
                OPEN P_RESULT FOR SELECT 'Invalid Email' AS MESSAGE FROM DUAL; RETURN;

            ELSIF ISVALIDEMAIL > 0 THEN
                SELECT NVL(COUNT(EMAIL),0) INTO VEMAIL FROM WEALTHMAKER.INVESTOR_MASTER WHERE UPPER(EMAIL) = UPPER(P_EMAIL_VALUE) AND SOURCE_ID != TRIM(SUBSTR(P_EXIST_CLIENT_CODE,1,8)) AND INV_CODE != TRIM(P_EXIST_CLIENT_CODE);  

                IF VEMAIL > 0 THEN
                    OPEN P_RESULT FOR SELECT 'Duplicate Email (Cannot be shared across different families)' AS MESSAGE FROM DUAL; RETURN;
                END IF;
            END IF;
        END IF;
    END IF ;
    ...
    IF P_MOBILE_NO_VALUE IS NOT NULL AND UPPER(P_NATIONALITY) NOT IN ('NRI') THEN
        ...
        ELSIF ISVALIDMOBILE = 1  THEN
            SELECT COUNT(MOBILE) INTO VMOBILE FROM WEALTHMAKER.INVESTOR_MASTER WHERE MOBILE = P_MOBILE_NO_VALUE AND SOURCE_ID != TRIM(SUBSTR(P_EXIST_CLIENT_CODE,1,8)) AND INV_CODE != TRIM(P_EXIST_CLIENT_CODE);

            IF VMOBILE > 0 THEN
                OPEN P_RESULT FOR SELECT 'Duplicate Mobile Number (Cannot be shared across different families)' AS MESSAGE FROM DUAL; RETURN;
            END IF;
        END IF;
    END IF;
   ...
   IF P_GUEST_CODE IS NOT NULL THEN 
    ...
    ELSE
        IF TRIM(P_RM_BUSINESS_CODE) IS  NOT NULL THEN
            ...
            IF V_GUEST_FPC_BRANCH > 0 THEN
                OPEN P_RESULT FOR 
                SELECT 'A valid guest code requried for FPC' AS MESSAGE FROM DUAL;
                RETURN;
            END IF;
        END IF;
    END IF;
    ...
    IF TRIM(P_RM_BUSINESS_CODE) IS NULL THEN
        OPEN P_RESULT FOR
            SELECT 'VALID BUSINESS RM REQUIRED' AS message FROM DUAL;RETURN;
    END IF;
    ...
    IF P_GUEST_CODE IS NOT NULL THEN 
        ...
        IF ISVALIDGUEST = 0 THEN
            OPEN P_RESULT FOR 
            SELECT 'INVALID GUEST CODE' AS message FROM DUAL; RETURN;
        END IF;

        SELECT NVL(COUNT(GUEST_CD),0) INTO V_GUESTCODE FROM CLIENT_MASTER  WHERE GUEST_CD = TRIM(P_GUEST_CODE);    
        IF V_GUESTCODE > 0 THEN
            -- Duplicate Guest Code found in CLIENT_MASTER
            OPEN P_RESULT FOR 
            SELECT 'Duplicate Guest Code' AS message FROM DUAL; RETURN;
        END IF;
    END IF;
    ...
    IF P_CLIENT_PAN IS NOT NULL THEN
        ISVALIDPAN := VALIDATEPAN(UPPER(TRIM(SUBSTR(P_CLIENT_PAN, 1, 10))));
        IF ISVALIDPAN IN ('InValid', 'Miss') THEN
            OPEN P_RESULT FOR SELECT 'Invalid PAN (e.g., AAAAA9999A)' AS message FROM DUAL; RETURN;
        ELSIF ISVALIDPAN = 'Valid' THEN
            ...
            IF VPAN > 0 THEN 
                OPEN P_RESULT FOR 
                SELECT 'Duplicate PAN' AS message FROM DUAL;
                RETURN;
            END IF;
        END IF;
    END IF;
    ...
    IF P_GUARDIAN_PERSON_PAN IS NOT NULL THEN       
        ...
        ELSIF ISVALID_GPAN = 'Valid' THEN
                ...
                ;
        OPEN P_RESULT FOR 
        SELECT 'Duplicate Guardian PAN (Cannot be shared across different families) ' || VG_PAN AS message FROM DUAL; RETURN;
            IF VG_PAN > 0 THEN
                OPEN P_RESULT FOR SELECT 'Duplicate Guardian PAN (Cannot be shared across different families)' AS message FROM DUAL; RETURN;
            END IF;
        END IF;
    end if;
    ...
    IF P_AADHAR_VALUE IS NOT NULL THEN 
        ...
        ELSIF ISVALID_AADHAAR = 'Valid' THEN          
            ...
            IF V_AADHAR > 0 THEN 
            OPEN P_RESULT FOR 
                SELECT 'Duplicate Aadhaar Number' AS message FROM DUAL; RETURN; 
            END IF;
        END IF;
    END IF;
    ...
    IF P_EMAIL_VALUE IS NOT NULL THEN
        ...
        IF UPPER(P_EMAIL_VALUE) NOT IN ('NA', 'NOT AVAILABLE', 'N/A','NONE') THEN
            ...
            ELSIF ISVALIDEMAIL = 1  THEN
                ...
                IF VEMAIL > 0 THEN
                    OPEN P_RESULT FOR SELECT 'Duplicate Email' AS message FROM DUAL; RETURN;
                END IF;
            END IF;
        END IF ;
    END IF;
    ...
    IF P_MOBILE_NO_VALUE IS NOT NULL AND UPPER(P_NATIONALITY) NOT IN ('NRI') THEN
        ...
        ELSIF ISVALIDMOBILE = 1  THEN
            ...
            IF VMOBILE > 0 THEN
                OPEN P_RESULT FOR SELECT 'Duplicate Mobile Number ' || P_MOBILE_NO_VALUE AS message FROM DUAL;
                RETURN;
            END IF;
        END IF;
    END IF;

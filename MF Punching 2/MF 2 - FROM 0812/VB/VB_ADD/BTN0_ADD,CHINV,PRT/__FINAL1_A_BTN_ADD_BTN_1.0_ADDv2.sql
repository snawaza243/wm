CREATE OR REPLACE PROCEDURE PSM_MF2_ADD_TR_PROCESS (
    ...
    PX_GLBINS_PREV_DATE     IN VARCHAR2,
    PX_GLBINS_NEXT_DATE     IN VARCHAR2,
    PX_GLBUP_PREV_DATE      IN VARCHAR2,
    PX_GLBUP_NEXT_DATE      IN VARCHAR2,    
    PX_CURSOR               OUT SYS_REFCURSOR
)
AS
    VP_MICRO_PAN_FLAG       NUMBER;
    V_CHK_ATM_TR            VARCHAR2(100) := NULL;
    V_SCH_CD_CLOSE          VARCHAR2(100);
    V_PX_HDN_PAN1           VARCHAR2(20);
    V_GENERATED_TR          VARCHAR2(100);
    P_GLBUP_PREVIOUSDATE    DATE;
    P_GLBUP_NEXTDATE        DATE;
    P_SUB_SIP               VARCHAR2(100);
    MYSWITCHSCHCODEA        VARCHAR2(100);
    v_err_msg               VARCHAR2(4000);
    

    /* PAYMENT_MODE KEY:VALUE
        C-->Cheque
        D-->Draft
        E-->ECS
        H-->Cash
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN -- CHECKING procedure IS PASS OR NOT , 1,2,3 DONE
        -- AUTHORIZATION CHECK
        IF PX_ROLE_ID NOT IN ('212') THEN        
            OPEN PX_CURSOR FOR SELECT 'YOU ARE NOT AUTHORIZED TO PUNCH THE TRANSACTION' AS MSG FROM DUAL;
            RETURN;
        END IF;
        
        -- REQ DT OR APPROVAL CHECK
        BEGIN
            IF TRIM(PX_DT_NUMBER) IS NOT NULL THEN
                DECLARE 
                    V_FN_CHECK_FOR_APPROVAL_ALL VARCHAR2(1);
                BEGIN
                    SELECT WEALTHMAKER.FN_CHECK_FOR_APPROVAL_ALL(PX_DT_NUMBER) INTO V_FN_CHECK_FOR_APPROVAL_ALL FROM DUAL WHERE ROWNUM = 1;
                    IF V_FN_CHECK_FOR_APPROVAL_ALL = '2' THEN
                        OPEN PX_CURSOR FOR SELECT 'APPROVAL REQUEST FOR THIS TRANSACTION HAS ALREADY BEEN RAISED' AS MSG FROM DUAL; 
                        RETURN;
                    ELSIF V_FN_CHECK_FOR_APPROVAL_ALL = '4' THEN
                        OPEN PX_CURSOR FOR SELECT 'APPROVAL REQUEST FOR THIS TRANSACTION HAS BEEN REJECTED BY MANAGEMENT' AS MSG FROM DUAL; 
                        RETURN;
                    END IF;
                END;
            ELSE
        ...
        IF PX_SIP_STP = 'SIP' THEN
            IF PX_SIP_END_DATE IS NULL OR PX_SIP_START_DATE IS NULL THEN
                OPEN PX_CURSOR FOR SELECT 'PLEASE ENTER SIP END DATE' AS MSG FROM DUAL; 
                RETURN;
            END IF;
        END IF;
        ...
        IF PX_INV_CODE IS NOT NULL AND PX_HDN_SCHEME1 IS NOT NULL AND PX_AMOUNT IS NOT NULL THEN
            ...
            BEGIN
                ...
                WHERE (A.ASA <> 'C' OR A.ASA IS NULL) 
                AND A.SCH_CODE = B.SCH_CODE 
                AND A.DOC_ID IS NOT NULL  
                AND A.CLIENT_CODE = PX_INV_CODE
                AND A.TR_DATE > SYSDATE - 90
                AND B.SCH_CODE = PX_HDN_SCHEME1 
                AND A.AMOUNT BETWEEN VP_AMT1 AND VP_AMT2;
        ...
        IF PX_AMOUNT IS NOT NULL THEN
            VP_MICRO_PAN_FLAG := 0;
            IF PX_SIP_TYPE = 'MICRO' AND TO_NUMBER(PX_AMOUNT) < 50000 THEN
                VP_MICRO_PAN_FLAG := 1;
            ELSE
                VP_MICRO_PAN_FLAG := 0;
            END IF;
        ...
        IF PX_DT_NUMBER IS NOT NULL THEN
            DECLARE 
                VP_VAL_COUNT NUMBER;
            BEGIN
                SELECT COMBO_PLAN_VAL(PX_DT_NUMBER, PX_HDN_SCHEME1, 0, 'MF') INTO VP_VAL_COUNT FROM DUAL;
        ...
        -- CHECK UNALLOCATED BRANCH AND BLOCK TO PUNCH            
        IF PX_BRANCH IS NOT NULL AND PX_BRANCH <> '' AND PX_BRANCH <> '0' THEN
            ...
            BEGIN
                ...
                IF V_BRANCH_COUNT > 0 THEN
                    OPEN PX_CURSOR FOR SELECT 'TRANSACTION CAN NOT BE PUNCHED IN UNALLOCATED BRANCH' AS MSG FROM DUAL;
                    RETURN;
                END IF;
            END;
        END IF;
        ...
        BEGIN
            ...
            BEGIN
                IF PX_BSS_CODE IS NOT NULL AND PX_BSS_CODE <> '' THEN
                    ...
                    IF NVL(VP_RESULT, 0) = 0 THEN
                        OPEN PX_CURSOR FOR
                            SELECT 'ERROR: UPLOADED BRANCH DT, ' || VP_ERRMSG AS MSG
                            FROM DUAL;
                        RETURN;
                    END IF;
                END IF;
        ...
        BEGIN
            DECLARE
                VP_FP_STATUS NUMBER;
            BEGIN
                IF PX_DT_NUMBER IS NOT NULL AND PX_INV_CODE IS NOT NULL THEN
                    SELECT FP_STATUS_CHECK1(PX_INV_CODE, PX_DT_NUMBER)
                    INTO VP_FP_STATUS
                    FROM DUAL;
        ...
        IF PX_TR_DATE IS NOT NULL THEN
            DECLARE
                ...
                GlbUpdatePre VARCHAR2(5);
                Glbupdatenext VARCHAR2(5);
                GlbINSPre VARCHAR2(5);
                GlbINSnext VARCHAR2(5);
                GlbUpdatePreType VARCHAR2(5);
                GlbupdatenextType VARCHAR2(5);
                GlbINSPreType VARCHAR2(5);
                GlbINSnextType VARCHAR2(5);
            BEGIN            
                ...
                IF VP_FRSAVE >= 1 AND VP_FRMODY = 0 THEN
                    IF (VP_ENTRY_DT < PX_GLBINS_PREV_DATE) OR (VP_ENTRY_DT > PX_GLBINS_NEXT_DATE) THEN
                        OPEN PX_CURSOR FOR
                ...
                IF VP_FRSAVE = 0 AND VP_FRMODY = 1 THEN
                    IF (VP_ORG_DT < PX_GLBUP_PREV_DATE) OR (VP_ORG_DT > PX_GLBUP_NEXT_DATE) THEN
                        OPEN PX_CURSOR FOR
                            SELECT 'Security restrictions for date range' AS MSG
                            FROM DUAL;
                        RETURN;
        ...
        OPEN PX_CURSOR FOR SELECT 'Pass 4' AS MSG FROM DUAL;
        RETURN;
        ...
        IF PX_DT_NUMBER IS NOT NULL THEN
            ...
            BEGIN
                ...
                INTO V_COUNT
                FROM TB_DOC_UPLOAD
                WHERE COMMON_ID = TRIM(PX_DT_NUMBER)
                AND TRAN_TYPE = 'MF';
        ...
        -- PX_TRANSACTION_TYPE AND PAYMENT_MODE 
        BEGIN
        ...
        -- REQUIRED FREQ AND INSTALLMENTS FOR SIP/STP
        ...
        -- REQUIRED PX_SIP_END_DATE, PX_SIP_AMOUNT AND PX_FRESH_RENEWAL FOR SIP 
        ...
        IF TRIM(PX_TRANSACTION_TYPE) = 'PURCHASE' THEN
            IF TRIM(PX_APP_NO) IS NOT NULL THEN
                IF LENGTH(PX_APP_NO) < 6 THEN
                    OPEN PX_CURSOR FOR
                        SELECT ' MINIMUM LENGTH OF APP NO SHOULD BE GREATER OR EQUAL TO 6 ' AS MSG FROM DUAL;
                    RETURN;
                ELSIF PX_APP_NO = '000000' THEN
                    OPEN PX_CURSOR FOR
                        SELECT ' PLEASE ENTER A VALID APP NO ' AS MSG FROM DUAL;
                    RETURN;
                END IF;
            END IF;
        END IF;
                ...
                IF NOT (PX_TRANSACTION_TYPE = 'SIP' AND P_SUB_SIP = 'MICROSIP') THEN
                    ...
                    IF PX_HDN_SCHEME1 <> 'OP#10826' THEN
                        IF VP_MICRO_PAN_FLAG = 0 THEN
                            IF TRIM(PX_PAN2) IS NULL OR NOT REGEXP_LIKE(PX_PAN2, '^[A-Z]{5}[0-9]{4}[A-Z]{1}$') THEN
                                OPEN PX_CURSOR FOR
        ...
        IF V_CHK_ATM_TR IS NOT NULL THEN
            ...
            BEGIN
                ...
                IF V_CHK_ATM_TR = '1' THEN
                    ...
                    IF NVL(TO_NUMBER(PX_AMOUNT), 0) < V_MIN_AMOUNT THEN
                        OPEN PX_CURSOR FOR
        ...
        IF PX_AMC IS NOT NULL AND PX_HDN_SCHEME1 IS NOT NULL AND PX_TR_DATE IS NOT NULL THEN
            ...
            BEGIN
                ...
                SELECT NVL(UPFRONT_RECD_NONANG_DETAIL(PX_AMC, PX_HDN_SCHEME1, TO_DATE(PX_TR_DATE, 'DD/MM/YYYY'), PX_AMOUNT, PX_TRANSACTION_TYPE, MYSWITCHSCHCODEA), 0)
                    + NVL(TRAIL_RECD_ANG_DETAIL(PX_AMC, PX_HDN_SCHEME1, TO_DATE(PX_TR_DATE, 'DD/MM/YYYY'), PX_AMOUNT), 0)
                INTO V_RECD_TOTAL
                FROM DUAL;

                -- Compare with entered expenses
                IF NVL(TO_NUMBER(PX_EXP_RS), 0) > V_RECD_TOTAL AND V_RECD_TOTAL <> 0 THEN
                    OPEN PX_CURSOR FOR
        ...
        IF PX_PAYMENT_MODE IN ('C', 'D', 'E', 'U','B') AND PX_FRESH_RENEWAL = 'F' THEN
            INSERT INTO TRANSACTION_MF_TEMP1 (
                ...
                V_CHK_ATM_TR, PX_SIP_AMOUNT, PX_INV_CODE, PX_BSS_CODE, PX_LOG_ID, PX_BSS_CODE, PX_BRANCH, V_PX_HDN_PAN1, PX_AMC, PX_HDN_SCHEME1, TO_DATE(PX_TR_DATE, 'DD/MM/YYYY'), PX_TRANSACTION_TYPE,
                PX_APP_NO, NVL(TO_DATE(PX_SIP_START_DATE,'DD/MM/YYYY'), NULL), PX_PAN2, PX_FOLIO_NO, PX_FROM_SWITCH_FOLIO, PX_HDN_SCHEME2_SWITCH, PX_PAYMENT_MODE, PX_BANK_NAME, PX_CHEQUE_NO, TO_DATE(PX_CHEQUE_DATE,'DD/MM/YYYY'),
                ...
                PX_FRESH_RENEWAL, PX_REGULAR_NFO, PX_DT_NUMBER, PX_SIP_TYPE, V_SCH_CD_CLOSE, PX_COB_CASE, PX_SWP_CASE, PX_FREEDOM_SIP 
            );
        ELSE
            INSERT INTO TRANSACTION_MF_TEMP1 (
                ATM_FLAG, SIP_AMOUNT, CLIENT_CODE, BUSINESS_RMCODE, LOGGEDUSERID, CLIENT_OWNER, BUSI_BRANCH_CODE, PANNO, MUT_CODE, SCH_CODE, TR_DATE, TRAN_TYPE,
        ...
        BEGIN
            ...
            AND (PANNO = V_PX_HDN_PAN1 OR (V_PX_HDN_PAN1 IS NULL AND PANNO IS NULL))
            AND ((PX_PAYMENT_MODE IN ('C', 'D', 'E', 'U','B') AND CHEQUE_NO = PX_CHEQUE_NO) OR (PX_CHEQUE_NO IS NULL AND CHEQUE_NO IS NULL))
            AND (APP_NO = PX_APP_NO OR (PX_APP_NO IS NULL AND APP_NO IS NULL))
            AND ROWNUM = 1;
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                V_GENERATED_TR := NULL;

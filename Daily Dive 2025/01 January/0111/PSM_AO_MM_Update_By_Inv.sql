...
BEGIN
    ...
    select pan 
    into v_current_db_pan
    from investor_master where inv_code = P_UPDATEBYCLIENTCODE;
    ...
    FROM (
        SELECT COUNT(*) AS cnt
        FROM client_master
        WHERE UPPER(pan) = UPPER(Trim(p_pan))
        ...
        SELECT COUNT(*) AS cnt
        FROM client_test
        WHERE UPPER(client_pan) = UPPER(Trim(p_pan))
    );

    if v_current_db_pan =  P_PAN then
        ...
        OPEN p_result FOR 
            SELECT 
                'Successfully Updated: ' || client_name ||
                ' (' || v_client_code ||
                ' / ' || (SELECT inv_code FROM investor_master WHERE inv_code = P_UPDATEBYCLIENTCODE)|| ')' AS message
            FROM 
                client_test
        ...
        if v_current_db_pan !=  P_PAN and v_pan_flag > 0 then

         OPEN p_result FOR 
            SELECT 'Duplidate Data: Pan is duplicate' AS message
        FROM dual;

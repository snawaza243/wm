create or replace PROCEDURE PSM_NPS_INSERT_UPDATE_PRA(
    ...
    currentRole number;
BEGIN
    -- Fetch the ISS_CODE for the provided P_SCHEME_CODE
    
    select '212' into currentRole from dual;
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            MyMutCode := NULL; -- Set MUT_CODE to NULL if no value is found
    END;

if currentRole = '212' then 

    IF P_MARK IS NULL THEN
        -- Perform INSERT operation into transaction_sttemp
        INSERT INTO transaction_sttemp (
            ...
            P_DATE,                       
            P_INVESTOR_CODE,   
            MyMutCode,                    -- Use the fetched ISS_CODE or NULL
            P_SCHEME_CODE,                           
            'PURCHASE',                  
            P_AMOUNT_INVESTED,           
            P_BUSINESS_BRANCH,          
        ...
        -- Fetch the CLIENT_CODE from transaction_sttemp using DOC_ID
        SELECT CLIENT_CODE INTO NewClientCode
        FROM transaction_sttemp
        WHERE DOC_ID = P_DT_NUMBER;

        -- Fetch the TRAN_CODE from transaction_sttemp using DOC_ID
        SELECT TRAN_CODE INTO MyTranCode
        FROM transaction_sttemp
        WHERE DOC_ID = P_DT_NUMBER;

        -- Insert into transaction_st table
        INSERT INTO transaction_st (
            ...
            P_AMOUNT_INVESTED,          
            P_BUSINESS_BRANCH,          
            SUBSTR(NewClientCode, 1, 8),                 -- Use the CLIENT_CODE fetched from transaction_sttemp               
            P_BUSINESS_RM,              
            P_BUSINESS_RM,
            P_BUSINESS_BRANCH,
            P_CHEQUE_NO,                
        ...
        -- Insert into nps_transaction table
        ...
        OPEN P_RESULT FOR
            SELECT 
                ts.CLIENT_CODE || ' and Tran Code Is ' || ts.TRAN_CODE AS STATUS
            FROM 
                transaction_st ts
            WHERE 
                ts.DOC_ID = P_DT_NUMBER;
    ...
    ELSE
        ...
        SET 
            ...
            MUT_CODE = MyMutCode,  -- Use the fetched ISS_CODE or NULL
        ...
        -- Additional updates to the nps_transaction table
        ...
        -- Return confirmation in P_RESULT
        OPEN P_RESULT FOR
            SELECT 'Record Updated' AS STATUS
            FROM DUAL;
    END IF;
else
        OPEN P_RESULT FOR

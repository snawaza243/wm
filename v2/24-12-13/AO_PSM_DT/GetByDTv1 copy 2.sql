...
BEGIN
    ...
    AND DU.COMMON_ID = TRIM(P_CLIENT_CODE);
        ...
        -- Calculate CT_COUNT
    SELECT NVL(COUNT(*), 0) INTO CM_COUNT 
    FROM TB_DOC_UPLOAD DU
    JOIN CLIENT_MASTER CM ON CM.CLIENT_CODE = SUBSTR(DU.INV_CODE,1,8)
    WHERE DU.VERIFICATION_FLAG = '1' 
    AND DU.REJECTION_STATUS = '0' 
    AND DU.TRAN_TYPE = 'AC' 
    AND DU.PUNCHING_FLAG = '0' 
    ...
    -- DT, IM exist, but CM not exist
    IF COMMON_FLAG > 0 AND INV_FLAG > 0 AND CM_COUNT = 0 THEN
    OPEN P_RESULT FOR
            SELECT 
            'False Data: DT, IM exist, but CM not exist' AS message,
            FROM DUAL;
    RETURN;  -- Exit the procedure here
    END IF;

    -- DT exist but CM, CT and IM not exist
    IF COMMON_FLAG > 0 AND INV_FLAG = 0 AND CT_COUNT = 0 AND ROW_COUNT = 0 THEN
        OPEN P_RESULT FOR
        SELECT 
            'DT exist but CM, TM and IM not exist' AS message,
            'False Data: DT, IM exist, but CM not exist' AS message,

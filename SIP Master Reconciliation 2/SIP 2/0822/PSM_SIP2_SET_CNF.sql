CREATE OR REPLACE PROCEDURE WEALTHMAKER.PSM_SIP2_SET_CNF(
...
BEGIN
    ...
    IF MyDispatch = 'N' THEN
        UPDATE TRANSACTION_MF_TEMP1 SET REC_FLAG = 'Y', RECO_DATE = SYSDATE, REC_USER = Glbloginid WHERE TRAN_CODE = MyTrCode;
        UPDATE TRANSACTION_MF_TEMP1 SET REC_FLAG = 'Y', RECO_DATE = SYSDATE, REC_USER = Glbloginid WHERE BASE_TRAN_CODE = MyTrCode;
        --Update Transaction_st@MF.BAJAJCAPITAL set REC_FLAG='Y', HO_TRAN_CODE=MyTrCode where tran_code in ( MyRtaTrCode );
    ELSE
        Update Transaction_mf_temp1 set  REC_FLAG='Y',RECO_DATE=TO_DATE(SYSDATE),REC_USER = Glbloginid WHERE TRAN_CODE = MyTrCode;
        Update Transaction_mf_temp1 set  REC_FLAG='Y',RECO_DATE=TO_DATE(SYSDATE),REC_USER = Glbloginid WHERE BASE_TRAN_CODE = MyTrCode;
        --Update Transaction_st@MF.BAJAJCAPITAL set REC_FLAG='Y',HO_TRAN_CODE=MyTrCode where tran_code in (MyRtaTrCode);
    END IF;
    ...
    OPEN P_CURSOR FOR 
    SELECT 'SUCCESS: Base SIP Registration Confirmed Successfully' AS message FROM dual;

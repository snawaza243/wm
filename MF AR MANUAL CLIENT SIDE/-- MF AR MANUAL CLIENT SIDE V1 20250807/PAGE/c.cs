...
namespace WM.Controllers
{
    public class MfManualReconciliationController
    {
        ...
        public (bool Success, string Message) SetReconcileTransactions(string trTranCode,string trTranType,string rtaTranCode,string rtaTranDate,string rtaFolio,decimal rtaAmount,string rtaDispatch)
        {
            ...
            using (OracleConnection conn = new OracleConnection(WebConfigurationManager.ConnectionStrings["ConnectionStringVM"].ConnectionString))
            {
                using (OracleCommand cmd = new OracleCommand("WEALTHMAKER.PSM_MF_RECO_M_RECONCILE2", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    cmd.Parameters.Add("P_TR_TRAN_TYPE", OracleDbType.Varchar2).Value = trTranType ?? (object)DBNull.Value;
        ...
        public (bool Success, string Message) SetTransactionRemark(string transactionCode, string remark)
        {
            ...
            using (OracleConnection conn = new OracleConnection(WebConfigurationManager.ConnectionStrings["ConnectionStringVM"].ConnectionString))
            {
                using (OracleCommand cmd = new OracleCommand("WEALTHMAKER.PMS_MF_RECO_M_SETREMARK", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public (bool Success, string Message) SetPMSATMConfirmation(string tranCode, string remarks, bool optPMS, bool optATM)
        {
            string loginId = pc.currentLoginID();
            string roleId = pc.currentRoleID();
            ...
            using (OracleConnection conn = new OracleConnection(WebConfigurationManager.ConnectionStrings["ConnectionStringVM"].ConnectionString))
            {
                using (OracleCommand cmd = new OracleCommand("WEALTHMAKER.PSM_MF_RECO_M_PMS_CNF2", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    cmd.Parameters.Add("P_USER", OracleDbType.Varchar2).Value = loginId ?? (object)DBNull.Value;
                    ...
                    try
                    {
                        ...
                        using (var reader = ((OracleRefCursor)outCursor.Value).GetDataReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                string message = reader["message"]?.ToString() ?? "No message returned";
                                bool isSuccess = !message.StartsWith("ERROR:", StringComparison.OrdinalIgnoreCase);

                                // Customize success message if needed
                                if (isSuccess)
                                {
                                    message = optPMS ? "PMS Record confirmed successfully"
                                           : optATM ? "ATM Record confirmed successfully"
                    ...
                    catch (OracleException ex)
                    {
                        ...
                        {
                            case 20001:
                                errorMessage = "Validation Error: Missing mandatory parameters (Transaction Code or User)";
                                break;
                            case 20002:
                                errorMessage = "No matching records found for the given transaction code";
                                break;
                            case 20003:
                                errorMessage = "No records found when updating ATM reconciliation flag";
                                break;
                            case 20004:
                                errorMessage = "Either PMS or ATM option must be selected";
                                break;
                        }
        ...
        public (bool Success, string Message) SetPMSATMUnConfirmation(string tranCode, string remarks, bool optPMS, bool optATM)
        {
            string loginId = pc.currentLoginID();
            string roleId = pc.currentRoleID();
            ...
            using (OracleConnection conn = new OracleConnection(WebConfigurationManager.ConnectionStrings["ConnectionStringVM"].ConnectionString))
            {
                using (OracleCommand cmd = new OracleCommand("WEALTHMAKER.PSM_MF_RECO_M_PMS_UNCNF2", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    try
                    {
                        ...
                        using (var reader = ((OracleRefCursor)outCursor.Value).GetDataReader())
                        {
                            if (reader.HasRows && reader.Read())
                            {
                                ...
                                // Customize success message for unconfirmation
                                if (isSuccess)
                                {
                                    message = optPMS ? "PMS Record unconfirmed successfully"
                                           : optATM ? "ATM Record unconfirmed successfully"
                                           : "Record unconfirmed successfully";
                                }
                    ...
                    catch (OracleException ex)
                    {
                        ...
                        switch (ex.Number)
                        {
                            ...
                            case 20002:
                                ...
                                break;
                            case 20003:
                                errorMessage = "No records found when removing ATM reconciliation flag";
                                break;
                            case 20004:
                                errorMessage = "Either PMS or ATM option must be selected";
                                break;
        ...
        #region OTHER FORM SIP DEPENDENT

CREATE OR REPLACE PROCEDURE WEALTHMAKER.PSM_MF_RECO_M_PMS_CNF2(
...
BEGIN
    ...
    IF P_OPT_PMS THEN
        -- Update for PMS option (by TRAN_CODE)
        ...
        -- Update for PMS option (by BASE_TRAN_CODE)
        UPDATE TRANSACTION_MF_TEMP1
        SET 
            REMARK_RECO = TRIM(P_REMARKS),
            REC_FLAG = 'Y',
    ...
    IF P_OPT_PMS THEN
        ...
        SELECT 'SUCCESS: ' || 
               ...
               v_count2 || ' record(s) updated in TRANSACTION_MF_TEMP1 (by BASE_TRAN_CODE)' AS message
        FROM dual;
    ELSE
        OPEN P_CURSOR FOR 
        SELECT 'SUCCESS: ' || 
               v_count1 || ' record(s) updated in TRANSACTION_MF_TEMP1 with ATM_RECO_FLAG' AS message
        FROM dual;
    END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        -- VALUES ('PSM_MF_RECO_M_PMS_CNF2', v_error_msg, SYSDATE, P_USER);
        -- COMMIT;
END;
/
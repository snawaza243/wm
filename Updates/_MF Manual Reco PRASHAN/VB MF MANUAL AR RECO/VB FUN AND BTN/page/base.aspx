...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script>
        ...
        function fetchTransactions() {
            debugger
            var data = {
                channel: $('#<%= ddlChannel.ClientID %>').val(),
                region: $('#<%= ddlRegion.ClientID %>').val(),
                zone: $('#<%= ddlZone.ClientID %>').val(),
                branch: $('#<%= ddlBranch.ClientID %>').val(),
                rm: $('#<%= ddlRM.ClientID %>').val(),
                dateFrom: $('#<%= txtDateFrom.ClientID %>').val(),
                dateTo: $('#<%= txtDateTo.ClientID %>').val(),
                amc: $('#<%= ddlAMC.ClientID %>').val(),
                ar: $('#<%= txtAR.ClientID %>').val(),
                reconciliationStatus: $('input[name="<%= rblReconciliation.UniqueID %>"]:checked').val(),
                cobFlag: $('#<%= cbCOB.ClientID %>').is(":checked") ? "1" : null,
                arNo: $('#<%= txtARNo.ClientID %>').val(),
                tranType: $('input[name="<%= rblTranType.UniqueID %>"]:checked').val(),
                registrar: $('input[name="<%= rblRegistrar.UniqueID %>"]:checked').val()
            };

            var single_channel=  $('#<%= ddlChannel.ClientID %>').val(),

            $.ajax({
                type: "POST",
                url: "MfManualReconciliation.aspx/GetTransactionData", // Replace with your actual page
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify(data),
                dataType: "json",
                success: function (response) {
                    if (response.d.success) {
                        var rows = response.d.data;
                        var $tbody = $('#transactionTable tbody');
                        //$tbody.empty();

                        $.each(rows, function (i, row) {
                            var html = "<tr>" +
                                "<td><input type='checkbox' class='select-row-checkbox' data-tran-code='" + row["TRAN_CODE"] + "' /></td>" +
                                "<td>" + row["TRAN_CODE"] + "</td>" +
                                "<td>" + formatDate(row["TR_DATE"]) + "</td>" +
    ...
    <style>
        /* .form-check-input {
            width: 5rem;Searching (Click Column You Want To Search)
        }*/
    </style>

     <%-- Table/ Grid row color css --%>
 <style>
     .hide-column {
         display: none;
     }
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <style>
                                                .rbl-options-common-margin label {
                                                    margin-right: 0.9rem; /* Adds space between radio items */
                                                }
                                            </style>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <div class="col-md-12 mt-2">
                                            <div class="d-flex align-items-center flex-wrap gap-3 mt-4">
                                                <asp:Button ID="btnGo" runat="server" Text="Go" CssClass="btn btn-sm btn-primary" OnClick="btnGo_Click" OnClientClick="fetchTransactions(); return false;"/>
...
if (ddlBranch && ddlBranch.querySelector("option[value='" + branch + "']")) {
    ...
    ddlBranch.selectedIndex = 0;
                                ...
                                <style>
                                    .draggable-container {
                                        /*max-height: 1250px;*/ /* Set a max height */
                            ...
                            <div >
                                ...
                                <div class="table table-bordered table-striped table-responsive" style="max-height: 250px; overflow-y: auto;">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="d-flex align-items-center flex-wrap gap-3 my-3">
                                            ...
                                            <asp:Button ID="btnReconcile" runat="server" CssClass="btn btn-sm btn-primary" Text="Reconcile" OnClick="btnReconcile_Click" />

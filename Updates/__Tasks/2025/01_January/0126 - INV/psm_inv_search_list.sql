...
BEGIN
    OPEN P_RESULT FOR
SELECT 
    IM.inv_code AS inv_code, IM.investor_name AS investor_name, IM.address1 AS address1, IM.address2 AS address2, NVL(CIM.city_name, '') AS CITY_ID, 
    IM.phone AS phone, NVL(BRM.branch_name, '') AS branch_CODE,  IM.mobile AS mobile, IM.dob AS dob, IM.pan AS pan,  NVL(EMM.rm_name, '') AS rm_CODE,  IM.source_id AS source_id
FROM 
    INVESTOR_MASTER IM
JOIN CLIENT_MASTER CM ON CM.CLIENT_CODE = IM.source_id

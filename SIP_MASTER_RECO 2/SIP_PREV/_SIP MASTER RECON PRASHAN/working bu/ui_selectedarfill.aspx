...
<script type="text/javascript">
    function onRowDoubleClick(row) {
        ...
        var rows = row.parentNode.querySelectorAll("tr");
        ...
        var hfTranCode = row.querySelector("[id*='hfTranCode']");
        if (hfTranCode) {
            // Store the transaction code in a hidden field for server-side access
            document.getElementById("<%= hftran1stcode.ClientID %>").value = hfTranCode.value;
            
            // Trigger postback
            __doPostBack("<%= tableSearchResults.UniqueID %>", "RowDoubleClick:" + row.rowIndex);
        }
    }
</script>
